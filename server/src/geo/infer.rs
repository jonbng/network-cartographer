    ...
    for hop in hops {
        ...
        if let Some(ip) = hop.addr {
            ...
            if let Some(meta) = cache.get(ip) {
                ...
                candidates = meta.hints.into_iter().filter(is_usable_city_hint).collect();
            }
        }
        ...
        let note = if chosen.is_none()
            && hop.addr.is_some()
            && !hop.addr.map(is_local_or_private).unwrap_or(true)
        {
...
fn pick_candidate(
    ...
    let candidates: Vec<&GeoHint> = candidates
        .iter()
        .filter(|c| is_usable_city_hint(c))
        .collect();
...
fn refine_path(hops: &mut [GeoHop]) {
    for i in 0..hops.len() {
        let prev_geo = if i > 0 { hops[i - 1].geo.clone() } else { None };
