...
fn path_reached_target(r: &TraceResult, target: IpAddr) -> bool {
    r.hops.iter().rev().any(|h| h.addr == Some(target))
}
...
fn commands_for(target: IpAddr, probe: &ProbeConfig) -> Vec<(String, Vec<String>)> {
    ...
    {
        // Quality order:
        // 1) TCP/443 — best through firewalls (often needs root)
        // 2) ICMP    — good quality when allowed
        // 3) UDP     — works unprivileged for most users
        // 4) tracepath fallback
        // Speed: -q 1, short waits, parallel probes (-N), modest max hops
        return vec![
            (
                "traceroute".into(),
                vec![
...
mod tests {
    ...
    #[cfg(target_os = "linux")]
    #[test]
    fn linux_commands_include_tcp_and_parallel() {
        let cmds = commands_for(target_v4(), &probe());
        assert!(cmds.len() >= 3);
        assert_eq!(cmds[0].0, "traceroute");
        assert!(cmds[0].1.iter().any(|a| a == "-T"));
